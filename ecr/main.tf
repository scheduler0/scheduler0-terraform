...
resource "null_resource" "docker_push" {
  provisioner "local-exec" {
    command = <<EOT
      ...
      docker build --no-cache -t scheduler-0-image -f ../Dockerfile ../
      docker tag scheduler-0-image:latest ${data.aws_caller_identity.current.account_id}.dkr.ecr.${data.aws_region.current.name}.amazonaws.com/${aws_ecr_repository.scheduler0_container_registry.name}:latest
